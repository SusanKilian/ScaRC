...
INTEGER, PARAMETER :: NSCARC_ERROR_MGM_PERMUTATION   = 23        !< Type of error message: matrix not symmetric
INTEGER, PARAMETER :: NSCARC_ERROR_MKL_CLUSTER       = 24        !< Type of error message: CLUSTER_SPARSE_SOLVER missing
INTEGER, PARAMETER :: NSCARC_ERROR_MKL_INTERNAL      = 25        !< Type of error message: internal error in MKL routine
INTEGER, PARAMETER :: NSCARC_ERROR_MKL_PARDISO       = 26        !< Type of error message: PARDISO solver missing
INTEGER, PARAMETER :: NSCARC_ERROR_MKL_STORAGE       = 27        !< Type of error message: wrong storage scheme in MKL
INTEGER, PARAMETER :: NSCARC_ERROR_MULTIGRID_LEVEL   = 28        !< Type of error message: wrong multigrid level
INTEGER, PARAMETER :: NSCARC_ERROR_PARSE_INPUT       = 29        !< Type of error message: wrong input parameter
...
INTEGER, PARAMETER :: NSCARC_ERROR_VECTOR_LENGTH     = 33        !< Type of error message: error in vector length
...
INTEGER, PARAMETER :: NSCARC_EXCHANGE_MGM_MEAN       = 13        !< Type of data exchange: MGM - Mean value interface
INTEGER, PARAMETER :: NSCARC_EXCHANGE_MGM_TRUE       = 14        !< Type of data exchange: MGM - True approximate interface
INTEGER, PARAMETER :: NSCARC_EXCHANGE_MGM_VELO       = 15        !< Type of data exchange: MGM - Velocity interface
INTEGER, PARAMETER :: NSCARC_EXCHANGE_NULLSPACE      = 16        !< Type of data exchange: nullspace entries (AMG only)
INTEGER, PARAMETER :: NSCARC_EXCHANGE_PRESSURE       = 17        !< Type of data exchange: pressure values
INTEGER, PARAMETER :: NSCARC_EXCHANGE_SOLIDS         = 18        !< Type of data exchange: pressure values
INTEGER, PARAMETER :: NSCARC_EXCHANGE_VECTOR_MEAN    = 19        !< Type of data exchange: mean values of a vector
INTEGER, PARAMETER :: NSCARC_EXCHANGE_VECTOR_PLAIN   = 20        !< Type of data exchange: plain values of a vector
INTEGER, PARAMETER :: NSCARC_EXCHANGE_ZONE_NEIGHBORS = 21        !< Type of data exchange: aggregation zones (AMG only)
INTEGER, PARAMETER :: NSCARC_EXCHANGE_ZONE_TYPES     = 22        !< Type of data exchange: aggregation zones types (AMG only)
...
INTEGER, PARAMETER :: NSCARC_MGM_DSCARC_TO_UHL_PREV  = 32        !< Type of MGM copy: diff ScaRC-UScaRC to prev unstruct hom Laplace
...
INTEGER, PARAMETER :: NSCARC_MGM_UHL_TO_PREV         = 36        !< Type of MGM copy: unstruct hom Laplace to prev 
INTEGER, PARAMETER :: NSCARC_MGM_OUHL_TO_PREV        = 37        !< Type of MGM copy: other unstruct hom Laplace to prev
...
SELECT CASE (TRIM(SCARC_METHOD))
   ...
   CASE ('MGM')
      ...
      SELECT CASE (TRIM(SCARC_MGM_INTERPOL))
         ...
         CASE DEFAULT
            CALL SCARC_ERROR(NSCARC_ERROR_PARSE_INPUT, SCARC_MGM_INTERPOL, NSCARC_NONE)
      END SELECT
...
OUHL_PREV => SCARC(NM)%LEVEL(NLEVEL_MIN)%MGM%OUHL_PREV
LL = 1
DO IOR0 = -3, 3
   IF (OL%GHOST_LASTW(IOR0) == 0) CYCLE
   UNPACK_MGM_TRUE: DO ICG = OL%GHOST_FIRSTE(IOR0), OL%GHOST_LASTE(IOR0)
      ...
      OUHL_PREV(IWG) = RECV_BUFFER_REAL(LL+1)
      LL = LL + 2
...
USE SCARC_VARIABLES
USE SCARC_TROUBLESHOOTING, ONLY: SCARC_ERROR
USE SCARC_STACK, ONLY: STACK, SCARC_SETUP_STACK, SCARC_SETUP_REFERENCES
...
MESHES_POISSON_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   SELECT_SCARC_METHOD: SELECT CASE (TYPE_METHOD)
      ...
      CASE (NSCARC_METHOD_MGM)
         ...
         CALL SCARC_SET_GRID_TYPE (NSCARC_GRID_UNSTRUCTURED)
         IF (SCARC_MGM_CHECK_LAPLACE .OR. SCARC_MGM_INIT_EXACT) THEN
            CALL SCARC_SETUP_POISSON (NM, NLEVEL_MIN)
            CALL SCARC_SETUP_BOUNDARY(NM, NLEVEL_MIN)
         ENDIF
