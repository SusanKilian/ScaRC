...
MESHES_LOOP: DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   DO IC = 1, G%NC
      ...
      DO ICOL = A%ROW(IC), A%ROW(IC+1) - 1
         ...
         ELSE
            WRITE(MSG%LU_VERBOSE, '(I6, A, 4I6, 6E14.6)') MY_RANK+1, ': CONNECTION: NO NEIGHBORS ', &
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   CALL SCARC_POINT_TO_MULTIGRID (NM, NL, NL+1)                       ! TODO: Check if correct coarse level is used
   AC => GC%POISSON
