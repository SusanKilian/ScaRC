...
INTEGER :: NM, NL
#ifdef WITH_MKL
INTEGER :: TYPE_SCOPE_SAVE, TYPE_MKL_SAVE
#endif
...
ELSE 
   ...
   TYPE_MKL_SAVE = TYPE_MKL(NLEVEL_MIN)
   ...
   IF (TYPE_MGM_LAPLACE == NSCARC_MGM_LAPLACE_PARDISO) THEN
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         CALL SCARC_POINT_TO_GRID (NM, NLEVEL_MIN)
         CALL SCARC_SETUP_MATRIX_MKL (NSCARC_MATRIX_LAPLACE, NM, NLEVEL_MIN)
         CALL SCARC_SETUP_BOUNDARY_MKL(NSCARC_MATRIX_LAPLACE, NM, NLEVEL_MIN)
      ENDDO 
   ENDIF

   IF (TYPE_MGM_LAPLACE == NSCARC_MGM_LAPLACE_OPTIMIZED) THEN 
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         CALL SCARC_POINT_TO_GRID (NM, NLEVEL_MIN)
         IF (L%STRUCTURED%NC > L%UNSTRUCTURED%NC) THEN
            CALL SCARC_SETUP_MATRIX_MKL (NSCARC_MATRIX_LAPLACE, NM, NLEVEL_MIN)
            CALL SCARC_SETUP_BOUNDARY_MKL(NSCARC_MATRIX_LAPLACE, NM, NLEVEL_MIN)
         ENDIF
      ENDDO 
   ...
   TYPE_MKL(NLEVEL_MIN) = TYPE_MKL_SAVE
...
IF (MY_RANK == 0) WRITE(*,*) 'BOUNDARY_MKL: IW:', IW
   GWC => G%WALL(IW)
   IOR0 = GWC%IOR
   IF (TWO_D .AND. ABS(IOR0) == 2) CYCLE       
   IF (GWC%BTYPE /= INTERNAL) CYCLE
...
IF (MY_RANK == 0) WRITE(*,*) 'BOUNDARY_MKL: IW, I,J,K, AS:', IW, I,J,K, AS%VAL(IP)
#ifdef WITH_SCARC_DEBUG
