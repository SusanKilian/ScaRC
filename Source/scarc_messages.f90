...
DO NM=LOWER_MESH_INDEX, UPPER_MESH_INDEX
   IF (MY_RANK == LASTID) CYCLE
   WRITE (MSG%FILE_DEBUG, '(A,A,i3.3)') TRIM(CHID),'.debug',MY_RANK+1
...
NLEN0 = MIN(10,NLEN)
IF (NLEN0 < 10) THEN
   MSG%CFORM1 = "( E14.6)"  ; WRITE(MSG%CFORM1(2:2),'(I1.1)') NLEN0
   MSG%CFORM4 = "( I5)"  ; WRITE(MSG%CFORM4(2:2),'(I1.1)') NLEN0
ELSE
   MSG%CFORM1 = "(  E14.6)" ; WRITE(MSG%CFORM1(2:3),'(I2.2)') NLEN0
   MSG%CFORM4 = "(  I5)" ; WRITE(MSG%CFORM4(2:3),'(I2.2)') NLEN0
ENDIF
IF (NLEN0+1 < 10) THEN
   MSG%CFORM2 = "( E14.6)"  ; WRITE(MSG%CFORM2(2:2),'(I1.1)') NLEN0+1
ELSE
   MSG%CFORM2 = "(  E14.6)" ; WRITE(MSG%CFORM2(2:3),'(I2.2)') NLEN0+1
ENDIF
IF (NLEN0+2 < 10) THEN
   MSG%CFORM3 = "( E14.6)"  ; WRITE(MSG%CFORM3(2:2),'(I1.1)') NLEN0+2
ELSE
   MSG%CFORM3 = "(  E14.6)" ; WRITE(MSG%CFORM3(2:3),'(I2.2)') NLEN0+2
ENDIF
...
INTEGER :: I, K, NM, IBAR0, JBAR0, KBAR0
...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   ELSE IF (NTYPE == 6) THEN
      ...
      WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MESHES(NM)%H(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
      WRITE(MSG%LU_DEBUG,*) 'HS'
      WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MESHES(NM)%HS(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
      WRITE(MSG%LU_DEBUG,*) 'PRHS'
      WRITE(MSG%LU_DEBUG,MSG%CFORM1) ((MESHES(NM)%PRHS(I,1,K), I=1, IBAR0), K=KBAR0,1,-1)
   ELSE IF (NTYPE == 5) THEN
      WRITE(MSG%LU_DEBUG,*) 'MGM%SCARC'
      WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%SCARC(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
      WRITE(MSG%LU_DEBUG,*) 'MGM%USCARC'
      WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%USCARC(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
      WRITE(MSG%LU_DEBUG,*) 'MGM%DSCARC'
      WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%DSCARC(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
   ELSE
      WRITE(MSG%LU_DEBUG,*) 'FVX'
      WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MESHES(MY_RANK+1)%FVX(I,1,K), I=0, IBAR0), K=KBAR0,0,-1)
      IF (IS_MGM) THEN
         WRITE(MSG%LU_DEBUG,*) 'MGM%SIP'
         WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%SIP(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
         WRITE(MSG%LU_DEBUG,*) 'MGM%UHL'
         WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%UHL(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
         IF (TYPE_MGM_BC == NSCARC_MGM_BC_EXPOL) THEN
            WRITE(MSG%LU_DEBUG,*) 'MGM%UHL2'
            WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%UHL2(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
         ENDIF
         IF (NTYPE >= 2) THEN
            WRITE(MSG%LU_DEBUG,*) 'MGM%UIP(.,1,.)'
            WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MGM%UIP(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
         ENDIF
         IF (NTYPE >= 1) THEN
            WRITE(MSG%LU_DEBUG,*) 'H'
            WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MESHES(MY_RANK+1)%H(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
            WRITE(MSG%LU_DEBUG,*) 'HS'
            WRITE(MSG%LU_DEBUG,MSG%CFORM3) ((MESHES(MY_RANK+1)%HS(I,1,K), I=0, IBAR0+1), K=KBAR0+1,0,-1)
         ENDIF
      ENDIF
   ENDIF
   IF (IS_MGM) THEN
      WRITE(MSG%LU_DEBUG,*) 'MGM%UVEL'
      WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MGM%UVEL(I,1,K), I=0, IBAR0), K=KBAR0,0,-1)
   ENDIF
   WRITE(MSG%LU_DEBUG,*) 'MESHES(MY_RANK+1)%U'
   WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MESHES(MY_RANK+1)%U(I,1,K), I=0, IBAR0), K=KBAR0,0,-1)
   WRITE(MSG%LU_DEBUG,*) 'MESHES(MY_RANK+1)%US'
   WRITE(MSG%LU_DEBUG,MSG%CFORM2) ((MESHES(MY_RANK+1)%US(I,1,K), I=0, IBAR0), K=KBAR0,0,-1)
ENDDO
...
SELECT CASE (NTYPE)
   ...
   CASE (NSCARC_DEBUG_WALL)
      ...
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         ...
         WRITE(MSG%LU_DEBUG,*) 'N_WALL_CELLS_EXT  =',L%N_WALL_CELLS_EXT
         WRITE(MSG%LU_DEBUG,*) 'N_WALL_CELLS_INT  =',L%N_WALL_CELLS_INT
         WRITE(MSG%LU_DEBUG,*)
