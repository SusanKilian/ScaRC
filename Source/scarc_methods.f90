...
STACK(NSTACK)%SOLVER%TYPE_GRID = NSCARC_GRID_UNSTRUCTURED
...
#endif
IF (MY_RANK == 0)   WRITE(*,*) '----------> MGM-START, TPI, PI =', TOTAL_PRESSURE_ITERATIONS, PRESSURE_ITERATIONS
...
IF (STATE_MGM /= NSCARC_MGM_SUCCESS) THEN
   ...
   MGM_CORRECTION_LOOP: DO ITE_MGM = 1, NIT_MGM
      ...
      IF (MY_RANK == 0)      WRITE(*,*) 'ITE_MGM, VEL_ERR:', ITE_MGM, VELOCITY_ERROR_GLOBAL
...
WRITE(MSG%LU_DEBUG,*) ' ===================== RELAX: FFT'
      ...
      DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
         ...
         IF (L%HAS_OBSTRUCTIONS) THEN
            ...
            WRITE(*,*) 'MY_RANK=',MY_RANK,' I am using MKL ', G%NC
            MKL => L%MKL
            AS => SCARC_POINT_TO_CMATRIX (NSCARC_MATRIX_POISSON_SYM)
...
WRITE(MSG%LU_DEBUG,*) 'MKL%ERROR:', MKL%ERROR
         ...
         ELSE
            ...
            WRITE(*,*) 'MY_RANK=',MY_RANK,' I am using FFT ', G%NC
            FFT => L%FFT
