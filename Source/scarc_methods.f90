...
CALL SCARC_MGM_STORE (NSCARC_MGM_POISSON)             ! store solution in MGM%SP
CALL SCARC_MGM_UPDATE_GHOSTCELLS (NSCARC_MGM_POISSON)

CALL SCARC_MGM_COPY (NSCARC_MGM_COPY_SP_TO_UL)                 ! first use MGM%SP as solution MGM%UP
...
ELSE
   ...
   ! Store ScaRC solution in MGM%SS, UScaRC soltution in MGM%US and difference of both in MGM%DSU
   ...
   IF (COMPARE_SCARC_VS_USCARC) THEN
      ...
      CALL SCARC_MGM_STORE (NSCARC_MGM_DIFF_HU_VS_HS)  ! build difference HD = HU - HS
   ...
   WRITE(MSG%LU_DEBUG,*) 'MGM-METHOD: VERY FIRST ITERATION, TPI=', TOTAL_PRESSURE_ITERATIONS, TYPE_MGM_BC
       ...
       CALL SCARC_MGM_COPY (NSCARC_MGM_COPY_DSU_TO_UL)   
       CALL SCARC_MGM_COPY (NSCARC_MGM_COPY_US_TO_UP)   
       ...
       IF (TYPE_MGM_BC == NSCARC_MGM_BC_EXPOL .AND. TOTAL_PRESSURE_ITERATIONS == 1) THEN
          CALL SCARC_MGM_COPY (NSCARC_MGM_COPY_UL_TO_ULP)
          CALL SCARC_MGM_COPY (NSCARC_MGM_COPY_OBDRY_TO_PREV)
       ENDIF
...
#endif
         CALL SCARC_MGM_COPY (NSCARC_MGM_DIFF_UL_VS_DSU)
         CALL SCARC_MGM_COPY (NSCARC_MGM_DIFF_UP_VS_US)
