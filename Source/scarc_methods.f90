...
DO NM = LOWER_MESH_INDEX, UPPER_MESH_INDEX
   ...
   DO IC = G%NONZERO, G%NC
      MGM%Y(IC) = MGM%B(IC)
      DO JC = 1, IC-1
         MGM%Y(IC) = MGM%Y(IC) - SCARC_EVALUATE_CMATRIX(LM, IC, JC) * MGM%Y(JC)
      ENDDO
   ENDDO
   ...
   DO IC = G%NC, 1, -1
      MGM%X(IC) = MGM%Y(IC)
      DO JC = IC+1, G%NC
         MGM%X(IC) = MGM%X(IC) - SCARC_EVALUATE_CMATRIX(UM, IC, JC) * MGM%X(JC)
      ENDDO
      MGM%X(IC) = MGM%X(IC)/SCARC_EVALUATE_CMATRIX(UM, IC, IC)
   ENDDO
#ifdef WITH_SCARC_DEBUG
